...
--
SELECT * FROM sql_cx_live.memebership t3
RIGHT JOIN sql_cx_live.users t4
ON t3.user_id = t4.user_id;
...
-- # set operators..
-- 1. UNION : 	•	Use UNION when you need unique rows (removes duplicates).
SELECT * FROM sql_cx_live.person1
UNION
SELECT * FROM sql_cx_live.person2;

-- 2. UNION ALL : Use UNION ALL when duplicates are acceptable (faster performance)
SELECT * FROM sql_cx_live.person1
UNION ALL
SELECT * FROM sql_cx_live.person2;

-- INTERSET Finds common rows in both tables

-- SELECT * FROM sql_cx_live.person1
-- INTERSECT
-- SELECT * FROM sql_cx_live.person1;

-- EXCEPT - Finds rows in the first table but not in the second
